...
final Map<DateTime, List<Event>> _kEventSource = <DateTime, List<Event>>{
  for (int item in List<int>.generate(50, (final int index) => index))
...
int getHashCode(final DateTime key) =>
    key.day * 1000000 + key.month * 10000 + key.year;

/// Returns a list of [DateTime] objects from [first] to [last], inclusive.
List<DateTime> daysInRange(final DateTime first, final DateTime last) {
  final int dayCount = last.difference(first).inDays + 1;
  return List<DateTime>.generate(
    dayCount,
    (final int index) =>
        DateTime.utc(first.year, first.month, first.day + index),
  );
}
